...
\usepackage{setspace}
\usepackage{csquotes}
<% if (german) { %>\usepackage[ngerman]{babel}<% } %><% if (english) { %>\usepackage[english]{babel}<% } %>
\usepackage{color}
\usepackage{hyperref}
...
\lstset{literate=%
   *{0}{{{\color{materialYellow}0}}}1
	...
	%Coversheet
  \newcommand{\headline}{<%= projectName %>}
  \newcommand{\subheadline}{<%= subheadline %>}
  ...
  <% if (includeAffidavit) { %>\input{affidavit.tex}<% } %>
